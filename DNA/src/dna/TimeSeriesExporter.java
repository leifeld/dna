...
public class TimeSeriesExporter {
	...
	boolean invertPersons, invertOrganizations, invertCategories;
	String agreement;
	boolean verbose;
	...
	public TimeSeriesExporter(
		...
		//infile
		DnaContainer dc = new ParseLatest(infile, verbose).getDc();
		this.sc = dc.sc;
		...
		if (invertPersons == true) {
			...
			for (int i = 0; i < pl.size(); i++) {
				...
				for (int j = 0; j < includedPersons.length; j++) {
					if (pl.get(i).equals(includedPersons[j])) {
						exclude = true;
					}
				}
				if (exclude == false) {
			...
			this.includedPersons = new String[pl2.size()];
			for (int i = 0; i < pl2.size(); i++) {
				this.includedPersons[i] = pl2.get(i);
			}
		} else if (includedPersons[0].equals("all")) {
			ArrayList<String> pl = dc.sc.getPersonList();
		...
		if (invertOrganizations == true) {
			...
			ArrayList<String> ol2 = new ArrayList<String>();
			for (int i = 0; i < ol.size(); i++) {
				boolean exclude = false;
				for (int j = 0; j < includedOrganizations.length; j++) {
					if (ol.get(i).equals(includedOrganizations[j])) {
						exclude = true;
					}
				}
				if (exclude == false) {
			...
			this.includedOrganizations = new String[ol2.size()];
			for (int i = 0; i < ol2.size(); i++) {
				this.includedOrganizations[i] = ol2.get(i);
			}
		} else if (includedOrganizations[0].equals("all")) {
			ArrayList<String> ol = dc.sc.getOrganizationList();
		...
		if (invertCategories == true) {
			...
			for (int i = 0; i < cl.size(); i++) {
				boolean exclude = false;
				for (int j = 0; j < includedCategories.length; j++) {
					if (cl.get(i).equals(includedCategories[j])) {
						exclude = true;
					}
				}
				if (exclude == false) {
			...
			this.includedCategories = new String[cl2.size()];
			for (int i = 0; i < cl2.size(); i++) {
				this.includedCategories[i] = cl2.get(i);
			}
		} else if (includedCategories[0].equals("all")) {
			ArrayList<String> cl = dc.sc.getCategoryList();
	...
	public boolean isActorIncluded(String actor) {
		if (persons == true) {
			for (int i = 0; i < this.includedPersons.length; i++) {
				if (this.includedPersons[i].equals(actor)) {
					return true;
				}
			}
		} else {
			for (int i = 0; i < this.includedOrganizations.length; i++) {
				if (this.includedOrganizations[i].equals(actor)) {
					return true;
				}
			}
		}
	...
	public boolean isCategoryIncluded(String category) {
		for (int i = 0; i < this.includedCategories.length; i++) {
			if (this.includedCategories[i].equals(category)) {
				return true;
			}
		}
		return false;
	...
	public void calculate() {
		...
		for (int i = sc.size() - 1; i >= 0; i--) {
			...
			if ( sc.get(i).getDate().before(start)
					...
					|| (sc.get(i).getAgreement().equals("yes") && this.agreement.equals("no"))
					|| (sc.get(i).getAgreement().equals("no") && this.agreement.equals("yes"))
					) {
	...
	private void csv() {
		try {
			...
			System.err.println("Error while saving CSV statistics file.");
		}
	}
