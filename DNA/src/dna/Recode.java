			...
			public void actionPerformed(ActionEvent e) {
				...
				if (dialog == 0) {
					for (int i = dna.Dna.mainProgram.dc.sc.size() - 1; i >= 0; i--) {
						boolean pcb = persCB.isSelected();
						boolean ocb = orgCB.isSelected();
						boolean ccb = catCB.isSelected();
						String o = (String)orgCombo.getSelectedItem();
						if (o == null) {
							o = "";
						}
						String p = (String)persCombo.getSelectedItem();
						if (p == null) {
							p = "";
						}
						String c = (String)catCombo.getSelectedItem();
						if (c == null) {
							c = "";
						}
						String dcp = dna.Dna.mainProgram.dc.sc.get(i).getPerson();
						String dco = dna.Dna.mainProgram.dc.sc.get(i).getOrganization();
						String dcc = dna.Dna.mainProgram.dc.sc.get(i).getCategory();
						if (
								(pcb && p.equals(dcp) && !ocb && !ccb) || // 1 0 0
								(!pcb && o.equals(dco) && ocb && !ccb) || // 0 1 0
								(!pcb && c.equals(dcc) && !ocb && ccb) || // 0 0 1
								(pcb && ocb && ccb && p.equals(dcp) && o.equals(dco) && c.equals(dcc)) || // 1 1 1
								(pcb && ocb && !ccb && p.equals(dcp) && o.equals(dco)) || // 1 1 0
								(pcb && !ocb && ccb && p.equals(dcp) && c.equals(dcc)) || // 1 0 1
								(!pcb && ocb && ccb && o.equals(dco) && c.equals(dcc)) // 0 1 1
								) {
							if (removeRadio.isSelected()) {
