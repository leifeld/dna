			...
			for (int k = 0; k < aitm.getRowCount(); k++) {
				...
				if ((Boolean)aitm.getValueAt(k, 0) == true) {
					try {
						...
						for (int l = 0; l < dna.Dna.mainProgram.dc.ac.getRowCount(); l++) {
							if (dna.Dna.mainProgram.dc.ac.get(l).getTitle().equals(title)) {
								duplicate = true;
							}
						}
						if (duplicate == true) {
							...
							for (int l = 0; l < dna.Dna.mainProgram.dc.ac.getRowCount(); l++) {
								Matcher m = p.matcher(dna.Dna.mainProgram.dc.ac.get(l).getTitle());
								boolean b = m.find();
								if (b == true) {
									count++;
								}
