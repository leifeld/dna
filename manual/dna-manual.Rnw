...
\usepackage{ltablex}
%\usepackage[table]{xcolor} %problems with knitr
\usepackage[
  unicode=true,
  pdfusetitle,
  bookmarks=true,
...
$java -version
@
If not, install it, e.\,g.\, via \code{apt-get}:
<<eval=FALSE, engine = 'bash', eval = FALSE, results = 'tex'>>=
...
\begin{enumerate}
  \item{
  Since the version of \R\ on the default repositories tends to be fairly outdated, we add the repository of the Comprehensive R Archive Network (CRAN) to the sources.list:
 <<eval=FALSE, engine = 'bash', eval = FALSE, results = 'tex'>>=
$sudo echo "deb https://cran.rstudio.com/bin/linux/ubuntu/artful/" 
$sudo tee -a /etc/apt/sources.list
@ 
  \textbf{Note, that you need to replace \code{/ubuntu/artful/} with your flavour and version of Linux}. Visit \href{https://cran.rstudio.com/bin/linux/}{CRAN} to see which ones are available
  }
  \item{
...
@ 
  ...
  \item{
  Now install \rstudio\ via gdebi (and install gdebi first if you don't already have it)\footnote{alternativly, you can download an installation file from \href{https://www.rstudio.com/products/rstudio/download/}{rstudio.com/products/rstudio/download/}.}:
<<eval=FALSE, engine = 'bash', eval = FALSE, results = 'tex'>>=
...
@
Since we only need one function from the package \texttt{devtools} at this point, it is not necessary to invoke the \texttt{library} command to load the whole package. Instead you can write \texttt{``devtools::''} and then type the function you want 
