...
public class OpenDnaFile implements Runnable {
	...
	public void run() {
		...
		try {
			...
			if (v.equals("1.16")) {
				...
				for (int i = 0; i < regex.getChildren().size(); i++) {
					...
					dna.Dna.mainProgram.regexTerms.add(rt);
				}
				...
				for (int i = 0; i < articles.getChildren().size(); i++) {
					...
					for (int j = 0; j < Dna.mainProgram.ac.getRowCount(); j++) {
						if (Dna.mainProgram.ac.getValueAt(j, 0).equals(title)) { //avoid duplicate article titles
				...
				for (int i = 0; i < duplicateStatements.size(); i++) {
					try {
						Dna.mainProgram.sc.addStatement(new Statement(
								Dna.mainProgram.sc.getFirstUnusedId(), 
				...
				if (convertDialog == 0) {
					...
					for (int i = 0; i < discourse.getChildren().size(); i++) {
						...
						for (Iterator<?> j = article.getContent().iterator(); j.hasNext( ); ) {
							...
							if (o instanceof Text) {
								...
								if (((Element)o).getName().equals("statement")) {
									...
									try {
										dna.Dna.mainProgram.sc.addStatement(new Statement(id, start, end, articleDate, text, title, person, organization, category, agreement));
									} catch (DuplicateStatementIdException e) {
										System.err.println("A statement with ID " + id + " already exists. It will not be added.");
									}
						...
						dna.Dna.mainProgram.ac.addArticle(art);
						progressMonitor.setProgress(i);
					}
		...
		if (progressMonitor.isCanceled()) {
			...
			if (dna.Dna.mainProgram.ac.getRowCount() > 0) {
				dna.Dna.mainProgram.currentFileName = filename;
				dna.Dna.mainProgram.currentFileLabel.setText( "Current file: " + dna.Dna.mainProgram.currentFileName );
			}
		}
		Dna.mainProgram.setRowSorterEnabled(true);
	}
