...
public class DnaExportWindow extends JFrame {
	...
	public DnaExportWindow() {
		...
		sCont = (StatementContainer)dna.Dna.mainProgram.sc.clone();
		exportWindow();
		resetAll();
	}
	...
	public void appearance() {
		...
		JPanel rightHalfPanel = new JPanel(new GridBagLayout());
		GridBagConstraints gbc2 = new GridBagConstraints();
		gbc2.gridx = 0;
		gbc2.gridy = 0;
		excludePanel = new ExcludePanel();
		rightHalfPanel.add(excludePanel, gbc2);
		
		gbc2.gridy = 1;
		gbc2.fill = GridBagConstraints.BOTH;
		buttonPanel = new ButtonPanel();
		rightHalfPanel.add(buttonPanel, gbc2);
		
		gbc2.gridy = 2;
		gbc2.fill = GridBagConstraints.HORIZONTAL;
		progressPanel = new ProgressPanel();
		rightHalfPanel.add(progressPanel, gbc2);
		
		gbc.gridx = 3;
		gbc.gridy = 0;
		gbc.gridheight = 4;
		basicOptionsPanel.add(rightHalfPanel, gbc);
	...
	public class FormatPanel extends JPanel {
		...
		JRadioButton csv, csvmatrix, mat, el, gml, comsql, son;
		...
		public FormatPanel() {
			...
			formatGroup.add(el);
			//add(el); //uncomment this line to activate the DL edgelist button
	...
	public class AlgorithmPanel extends JPanel {
		...
		JRadioButton sl, el, affil, xSec, tWind, atten, sonia;
	...
	public class ProgressPanel extends JPanel {
		...
		public void reset() {
			log.setText("This is the export log.\n\nReady to export a network file...");
		}
	}
	...
	class fileExporter extends Thread {
		...
		public void run() {
			...
			if (outfile.equals("null")) {
				...
				progressPanel.reset();
				progressPanel.log.setText("(1/4) Applying exclude filter... ");
				...
				//apply exclude list
				int oldSize = sc.statements.size();
				...
				int newSize = sc.statements.size();
				progressPanel.log.append("Keeping " + newSize + " out of " + oldSize + " statements.");				
				...
				//keep backup copy of all vertices in case the include-isolates option is activated
				StatementContainer sc2 = (StatementContainer)sCont.clone();
				if (formatPanel.el.isSelected() || formatPanel.mat.isSelected()) {
					sc2.reformatDl();
				} else {
					sc2.reformat();
				}
				persIsolates = (ArrayList<String>) sc2.getPersonList().clone();
				orgIsolates = (ArrayList<String>) sc2.getOrganizationList().clone();
				catIsolates = (ArrayList<String>) sc2.getCategoryList().clone();
				
				//apply date filter
				progressPanel.log.append("\n(2/4) Applying date filter... ");
				...
				progressPanel.log.append("Keeping " + sc.statements.size() + " out of " + newSize + " statements.");
				...
				if (algorithmPanel.sl.isSelected()) {
					progressPanel.log.append("\n(3/4) List of statements has been compiled.");
				} else {
					progressPanel.log.append("\n(3/4) Graph with " + graph.countVertices() + " vertices and " + graph.countEdges() + " edges has been created.");
				}
				exportControl();
				progressPanel.log.append("\n(4/4) File has been exported.");
			}
			...
			try {
				...
				for (int i = 0; i < sc.size(); i++) {
					...
					br.write(sc.get(i).getId() + ";"
							+ sc.get(i).getArticleTitle().replaceAll(";", ":") + ";"
							+ sc.get(i).getStart() + ";"
							+ sc.get(i).getStop() + ";"
							+ g.get(Calendar.YEAR) + "-" 
		...
		public void exportFilterSonia (String outfile) {
			...
			try {
				...
				System.out.println("Error while saving SoNIA file.");
			}
		}
		...
		private void exportGraphMl(String outfile) {
			try {
				...
				if (algorithmPanel.affil.isSelected()) {
					...
					out.write("<key id=\"weight\" for=\"edge\" attr.name=\"weight\" attr.type=\"double\"/>");
				}
				out.newLine();
				out.newLine();
				...
				if (algorithmPanel.affil.isSelected()) {
					for (int i = 0; i < graph.e.size(); i++) {
						...
						out.newLine();
						if (!coOccurrencePanel.ignoreDuplicates.isSelected()) {
							out.write(" <data key=\"weight\">" + (int)graph.e.get(i).getWeight() + "</data>");
							out.newLine();
						}
						out.write(" </edge>");
					}
