...
public class SearchWindow extends JFrame {
	...
	public SearchWindow() {
		...
		this.setIconImage(searchIcon.getImage());
		...
		JButton searchRevert = new JButton(searchRevertIcon);
		...
		JLabel hint = new JLabel("<html>More information: <a href=\"http://www.regular-expressions.info\">http://www.regular-expressions.info</a></html>");
		...
		resultScroller.setPreferredSize(new Dimension(900, 180));
		resultTable.getColumnModel().getColumn( 0 ).setPreferredWidth( 80 );
		resultTable.getColumnModel().getColumn( 1 ).setPreferredWidth( 220 );
		...
		searchPanel.add(resultScroller, BorderLayout.CENTER);
	...
	class ReportGenerator implements Runnable {
		...
		public void run() {
			...
			progressMonitor = new ProgressMonitor(SearchWindow.this, "Searching...", "", 0, Dna.mainProgram.ac.getRowCount() - 1 );
			...
			for (int i = 0; i < Dna.mainProgram.ac.getRowCount(); i++) {
				...
				while (m.find()) {
					...
					int end;
					if (m.end() < text.length() - 30) {
						end = m.end() + 30;
					} else {
						end = text.length();
					}
					String subtext = "..." + text.substring(start, end) + "...";
					SearchResult sr = new SearchResult(i, Dna.mainProgram.ac.get(i).getTitle(), date, m.start() + 1, m.end() + 1, subtext);
