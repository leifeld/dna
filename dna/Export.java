...
public class Export extends JFrame {
	...
	public class AlgorithmPanel extends JPanel {
		...
		public AlgorithmPanel() {
			...
			algoGroup.add(el);
			add(el);
			sl = new JRadioButton("List of statements", false);
		...
		private void exportGraphMl(String outfile) {
			...
			try {
				...
				if (algorithmPanel.affil.isSelected()) {
					...
					if (coOccurrencePanel.ignoreDuplicates.isSelected() && agreementPanel.comb.isSelected()) {
						out.newLine();
						out.write("<key id=\"agreement\" for=\"edge\" attr.name=\"agreement\" attr.type=\"string\"/>");
					}
				} else {
				...
				if (algorithmPanel.affil.isSelected()) {
					for (int i = 0; i < graph.e.size(); i++) {
						...
						if (coOccurrencePanel.ignoreDuplicates.isSelected() && agreementPanel.comb.isSelected()) {
							String agree;
							if (graph.e.get(i).getWeight() == 1) {
								agree = "yes";
							} else if (graph.e.get(i).getWeight() == 2) {
								agree = "no";
							} else if (graph.e.get(i).getWeight() == 3) {
								agree = "both";
							} else {
								agree = "NA";
							}
							out.write("<data key=\"agreement\">" + agree + "</data>");
							out.newLine();
						}
						out.write(" </edge>");
					}
				...
				System.err.println("Error while saving GraphML file.");
			}
		}
