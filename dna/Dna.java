 ...
 * 
 * done:
 * - prohibit two search threads at the same time
 * - disable search button while searching
 * - bugfix: saving upon close killed the file
 * - fixed regex bug in Linux; expressions are now directly highlighted after adding them
 * - statement filter takes less space
 * - statement list has been replaced by a sidebar where many functions now reside
 * - toolbar replaced by menu
 * - collapsible full-text search panel
 * 
 * to do:
 ...
 * - replace statement popups by a collapsible version at the bottom
 * 
 * @author Philip Leifeld
 * @version 1.20 - 17 July 2010
 */
public class Dna extends JFrame {
	...
	private void statementTable() {
        ...
        contradictionTaskPane.setName("Find contradictions");
        contradictionTaskPane.setTitle("Find contradictions");
	...
	public void textArea() {
		...
		searchPanelButton.addActionListener(collapsiblePane.getActionMap().get(JXCollapsiblePane.TOGGLE_ACTION));
	}
	...
	private void menu() {
		...
		JMenuItem closeFile = new JMenuItem("Close", closeIcon);
		...
		searchPanelButton = new JMenuItem("Show/hide full-text search", searchPanelIcon);
