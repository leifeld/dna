...
public class Dna extends JFrame {
	...
	JTextField textField, personField, organizationField, categoryField;
	JRadioButton textButton, allButton, articleButton, personButton, organizationButton, categoryButton;
	JCheckBox editMode;
		...
		try {
			RowFilter<StatementContainer, Integer> articleFilter = new RowFilter<StatementContainer, Integer>() {
    			public boolean include(Entry<? extends StatementContainer, ? extends Integer> entry) {
    				StatementContainer stcont = entry.getModel();
    				Statement st = stcont.get(entry.getIdentifier());
    				Pattern p = Pattern.compile(title);		//TODO: this must be changed from a pattern matcher to String.equals() because articles with brackets are not found anymore! 
    				Matcher m = p.matcher(st.getArticleTitle());
    				boolean b = m.find();
    				if (b == true) {
        				return true;
        			}
		...
		try {
			...
			if (v.equals("1.09")) {
				...
				for (int i = 1; i < discourse.getChildren().size(); i++) {
					...
					//articleText = articleText.replaceAll(ls, "<br />");
				    ...
				    for (int j = 0; j < statementsXml.getChildren().size(); j++) {
				    	...
				    	try {																//TODO: hier eine automatische Reparaturfunktion einbauen!
	...
	public class ToolTip extends JFrame {
		...
		public void showToolTip() {
			...
			person = new JComboBox(persListe.toArray());
			...
			organization = new JComboBox(orgListe.toArray());
