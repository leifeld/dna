 ...
 * done: 
 * - resizable layout
 * - check for article and statement duplicates when opening DNA files and rename them/their IDs
 * - bugfix: after clicking on "show all statements", one can now open and import files without exceptions being displayed.
 * - SoNIA algorithm for affiliation networks
 * - full-text search function across articles
 * 
 * to do:
 * - check DNA files for strange symbols, double spaces etc.; this could be a problem because people use the XML import format
 * - field for the article type
 * - combine statement filters by check boxes
 * - there is a bug with the regex highlighting under Linux
 * - command-line feature
 * - new layout based on SwingX and a menu structure
 * 
 * @author Philip Leifeld
 * @version 1.19 - 15 June 2010
 */
public class Dna extends JFrame {
	...
	public Dna() {
		...
		statementSplitPane.add(statementPanel, BorderLayout.EAST);
	...
	public class RegexManager extends JFrame {
		...
		public RegexManager() {
			...
			listScroller.setPreferredSize(new Dimension(50, 200));
			...
			panel.add(newFields, BorderLayout.CENTER);
	...
	class SearchPanel extends JPanel {
		...
		public SearchPanel() {
			...
			String searchToolTip = "<html>You can enter a regular expression in the search <br/>field and click on the arrow buttons to go to the <br/>previous or next occurrence in the text. This <br/> works only in the current article. There is another <br/> full-text search function for the whole file in the toolbar.</html>";
	...
	private void statementTable() {
		...
		sp = new SearchPanel();
		statementPanel.add(sp, BorderLayout.NORTH);
		...
		if (articleTable.getRowCount() == 0) {
			...
			if ( question == JOptionPane.YES_OPTION ) {
				if ( currentFileName.equals("") ) {
					...
					Thread save = new Thread( new SaveDnaFile( currentFileName ), "Save DNA file on exit" );
					save.start();
				}
				System.exit(0);
	...
	private void saveAsDialog() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			Thread save = new Thread( new SaveDnaFile( currentFileName ), "Save DNA file" );
			save.start();
		...
		if (articleTable.getRowCount() == 0) {
			...
			if ( question == JOptionPane.YES_OPTION ) {
				if ( currentFileName.equals("") ) {
					...
					Thread save = new Thread( new SaveDnaFile( currentFileName ), "Save and close current DNA file" );
					save.start();
				}
				clearSpace();
			} else if ( question == JOptionPane.NO_OPTION ) {
	...
	private void buttons() {
		...
		save.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if ( currentFileName.equals("") ) {
					...
					Thread save = new Thread( new SaveDnaFile( currentFileName ), "Save" );
					save.start();
				}
			}
		});
		...
		//Button: full-text search
		Icon searchIcon = new ImageIcon(getClass().getResource("/icons/find.png"));
		JButton searchButton = new JButton(searchIcon);
		searchButton.setToolTipText( "a full-text search function across all articles..." );
		...
		//Button: user-defined regex terms
		Icon regexIcon = new ImageIcon(getClass().getResource("/icons/color_swatch.png"));
		JButton regexButton = new JButton(regexIcon);
		regexButton.setToolTipText( "manage regular expressions that are highlighted in the text..." );
		...
		//Button: contradiction report
		Icon contrPersIcon = new ImageIcon(getClass().getResource("/icons/user.png"));
		JButton contrPersButton = new JButton(contrPersIcon);
		contrPersButton.setToolTipText( "report within-person contradictions" );
		toolbar.add(contrPersButton);
		contrPersButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new ContradictionReporter(true);
			}
		});

		//Button: contradiction report
		Icon contrOrgIcon = new ImageIcon(getClass().getResource("/icons/group.png"));
		JButton contrOrgButton = new JButton(contrOrgIcon);
		contrOrgButton.setToolTipText( "report within-organization contradictions" );
		toolbar.add(contrOrgButton);
		contrOrgButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new ContradictionReporter(false);
			}
		});
