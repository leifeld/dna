 ...
 * 
 * change log:
 * - bugfix: out of memory error is now handled correctly
 * - bugfix: stacktrace is displayed when JDOM exception is encountered
 * - bugfix: text window does not jump to the bottom anymore after inserting or removing statements
 * - added an improved progress bar to the network export window
 * - redesigned the terminal output in the network export window
 * - error messages now appear in red color in the terminal
 * - time necessary for loading the network export window has been decreased
 * - search function
 * - import function was broken for dna files of version 1.16
 * 
 * @author Philip Leifeld
 * @version 1.17 - 09 March 2010
 */
		...
		public void parseArticles(String filename) {
			try {
				...
				if (v.equals("1.16")) {
					...
					for (int i = 1; i < articles.getChildren().size(); i++) {
