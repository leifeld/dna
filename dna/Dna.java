...
public class Dna extends JFrame {
	...
	public class Recode extends JFrame {
		...
		JRadioButton renameRadio, removeRadio, swapRadio, persRadio, orgRadio, catRadio;
		...
		public Recode() {
			...
			renameRadio = new JRadioButton("rename to ", true);
			removeRadio = new JRadioButton("remove all matching statements completely", false);
			swapRadio = new JRadioButton("swap actor fields (person <-> organization)", false);
			bg1.add(renameRadio);
			bg1.add(removeRadio);
			bg1.add(swapRadio);
			...
			okButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (removeRadio.isSelected()) {
						...
						operation = "rename";
						into = " to \"" + textField.getText() + "\"";
					} else if (swapRadio.isSelected()) {
						operation = "change";
					}
					...
					if (dialog == 0) {
						for (int i = sc.size() - 1; i >= 0; i--) {
							if ( persRadio.isSelected() && removeRadio.isSelected() && persCombo.getSelectedItem().equals(sc.get(i).getPerson())
								...
								counter++;
							} else if ( (swapRadio.isSelected() && persRadio.isSelected() && persCombo.getSelectedItem().equals(sc.get(i).getPerson()))
									|| (swapRadio.isSelected() && orgRadio.isSelected() && orgCombo.getSelectedItem().equals(sc.get(i).getOrganization())) ) {
								String p = sc.get(i).getPerson();
								sc.get(i).setPerson(sc.get(i).getOrganization());
								sc.get(i).setOrganization(p);
								counter++;
							}
						}
						dispose();
			...
			JPanel outerSelectionPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0));
			...
			operationPanel.add(removeRadio);
			JPanel renamePanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0));
			renamePanel.add(renameRadio);
			renamePanel.add(textField);
			operationPanel.add(renamePanel);
			operationPanel.add(swapRadio);
