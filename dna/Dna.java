 ...
 * 
 * to do:
 * - check DNA files for consistency and duplicate article titles etc.; this could be a problem because people use the XML import format
 * - search function over several articles
 * - filter for articles
 * - field for the article type
 * - combine statement filters by check boxes
 * - there is a bug with the regex highlighting
 * - command-line feature
 * 
 * @author Philip Leifeld
 * @version 1.18 - 05 June 2010
 */
public class Dna extends JFrame {
	...
	private void statementTable() {
		...
		String searchToolTip = "<html>You can enter a regular expression in the search <br/>field and click on the arrow buttons to go to the <br/>previous or next occurrence in the text.</html>";
		...
		searchPanel.add(searchRevert, BorderLayout.WEST);
		...
		statementPanel.add(searchPanel, BorderLayout.NORTH);
		...
		if (articleTable.getRowCount() == 0) {
			...
			if ( question == JOptionPane.YES_OPTION ) {
				if ( currentFileName.equals("") ) {
					...
					Thread save = new Thread( new SaveDnaFile( currentFileName ) );
					save.start();
				}
				System.exit(0);
		...
		xmlOpen.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					File file = fc.getSelectedFile();
					try {
						Thread open = new Thread( new OpenDnaFile( file.getPath() ) );
						open.start();
